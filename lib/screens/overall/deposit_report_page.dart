...
class DepositReportPage extends StatefulWidget {
  ...
  const DepositReportPage({Key? key, this.selectDate}) : super(key: key);

  @override
  _DepositReportPageState createState() => _DepositReportPageState();
  // _SynchronizedScrollTablesState();
}
          ...
          for (var summary in filteredSummaries) {
            ...
            creditcards?.forEach((creditcardType, creditcardData) {
              if (creditcardData is List) {
                for (var creditcard in creditcardData) {
                  ...
                  totalCreditCardsByType[creditcardType]!['Total'] = (totalCreditCardsByType[creditcardType]!['Total'] ?? 0.0) + creditcardValue;
            ...
            if (discounts.isNotEmpty) {
              ...
              for (var discount in discounts) {
                // คำนวณหรือทำการต่างๆ กับ discount
                var discountName = discount['DiscountName'];
                var discountValue = discount['Value'] ?? 0.0;

                print("Discount Name: $discountName, Discount Value: $discountValue");
              }
            } else {
              // ถ้า discounts เป็น [] หรือไม่มีข้อมูล
            ...
            salebyordertypes?.forEach((salebyordertypeType, salebyordertypeData) {
              if (salebyordertypeData is Map) {
                // ดึงข้อมูลจาก 'Total' ซึ่งเป็น List
                ...
                for (var salebyordertype in salebyordertypeTotal) {
                  ...
                  // ถ้ามีค่า Value ที่ไม่ใช่ 0 ก็เพิ่มเข้าไปใน totalSalebyOdertypes
                          ...
                          DataColumn(
                            label: SizedBox(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(fontFamily: 'Inter', fontSize: 14, color: '#3C3C3C'.toColor(), fontWeight: FontWeight.w600),
                              ),
                            ),
                          ),
                          ...allDates.map(
                          ...
                          ...paymentTotals.entries.map((entry) {
                            ...
                            final rowValues = allDates.map((date) {
                              final dailyTotal = filteredSummaries.where((summary) {
                                final summaryDate = DateTime.parse(summary['Date']).toLocal();
                                final formattedDate = "${summaryDate.month.toString().padLeft(2, '0')}/${summaryDate.day.toString().padLeft(2, '0')}";
                          ...
                          ...totalSalebyOdertypeByType.entries.map((entry) {
                            ...
                            final rowValues = allDates.map((date) {
                              final dailyTotal = filteredSummaries.where((summary) {
                                ...
                                return sum + (salebyordertypes[salebyordertypeType]?.fold(0.0, (s, item) => s + (item['Value'] ?? 0.0)) ?? 0.0);
                              });
                              return DataCell(Text(dailyTotal == 0.0 ? "\$XX.XX" : "\$${dailyTotal.toStringAsFixed(2)}"));
                            }).toList();
                            return DataRow(
